...
class Log:
    ...
    def get_file_sorted(self,filepath):
    ...
    def handle_logs(self):
        ...
        for dir in dir_list:
            ...
            if file_list: #目录下没有日志文件
                for i in file_list:
                    file_path = os.path.join(dirPath,i) #拼接文件的完整路劲
                    t_list = self.TimeStampToTime(os.path.getctime(file_path)).split('-')
                    now_list = self.TimeStampToTime(time.time()).split('-')
                    t = datetime.datetime(int(t_list[0]),int(t_list[1]),int(t_list[2])) #将时间转换成datetime类型
                ...
                if len(file_list) > 4 :#限制目录下记录文件的数量
    ...
    def __console(self,level,message):
        ...
        fh = RotatingFileHandler(filename=self.logName,mode='a',maxBytes=1024 * 1024 * 5,backupCount=5,encoding='utf-8') #滚动备份日志
